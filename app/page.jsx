...
const s = {
  ...
  btn: (variant) => ({
    ...
    borderRadius:8, fontWeight:600, fontSize: variant==="sm" ? 12 : 14, cursor:"pointer",
  ...
  const generate = async () => {
    ...
    try {
      const text = await callClaude({
        ...
        prompt: `Generate 6 YouTube video title ideas about: ${topic}`,
      });
      const parsed = JSON.parse(text.replace(/```json|```/g,"").trim());
      setIdeas(parsed);
    } catch(e) {
      console.error(e);
      setError("Couldn't generate ideas. Check your API key and try again.");
    }
    setLoading(false);
  };
      ...
      <div style={s.card}>
        ...
        {ideas.map((idea,i) => (
          <div key={i} style={{display:"flex", alignItems:"center", gap:12, padding:"10px 0", borderBottom: i<ideas.length-1?`1px solid ${C.border}`:"none"}}>
            ...
            <button style={s.btn("sm")} onClick={()=>{}}>Use</button>
          </div>
        ))}
      </div>
    </div>
  ...
  const generate = async () => {
    ...
    try {
      const text = await callClaude({
        ...
        prompt: `Write a YouTube video script for: "${title}"`,
      });
      setScript(text);
    } catch(e) {
      console.error(e);
      setError("Couldn't generate script. Check your API key and try again.");
    }
    setLoading(false);
  };
    ...
    <div>
      ...
      <div style={s.card}>
        ...
        {checks.map((c,i) => {
          ...
          return (
            <div key={i} onClick={()=>toggle(i)} style={{display:"flex", alignItems:"center", gap:12, padding:"11px 0", borderBottom:i<checks.length-1?`1px solid ${C.border}`:"none", cursor:"pointer"}}>
              <div style={{
              ...
              <div style={{flex:1, fontSize:13, color: isDone ? C.muted : C.text, textDecoration: isDone ? "line-through" : "none"}}>
                {c}
              </div>
            </div>
          );
      ...
      {done.length === checks.length && (
        <div style={{...s.card, marginTop:14, background:C.accentDim, borderColor:C.accent}}>
          <div style={{fontSize:14, color:C.accent, fontWeight:600, textAlign:"center"}}>
  ...
  return (
    <div style={s.app}>
      <aside style={s.sidebar}>
        <div style={s.brand}>
          <div style={s.brandName}>✦ CreatorOS</div>
          <div style={s.brandSub}>Finance Channel</div>
        </div>
        <nav style={s.nav}>
          {NAV.map(item => (
        ...
        <div style={s.postDays}>
          <div style={{fontSize:10, color:C.muted, letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:8}}>
            Next Post
          </div>
          <div style={{fontSize:13, color:C.text, fontWeight:500}}>Wed, Apr 23</div>
          <div style={{fontSize:11, color:C.accent, marginTop:2}}>in 2 days</div>
        </div>
      </aside>
      <main style={s.main}>
        {PAGES[page]}
